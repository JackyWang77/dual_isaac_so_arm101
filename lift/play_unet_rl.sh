#!/bin/bash
# Play/Test UnetPolicy + Residual RL - Lift task
cd "$(dirname "$0")/.."
set -e
...
if [ -z "$PRETRAINED_CHECKPOINT" ]; then
    PRETRAINED_CHECKPOINT=$(ls -t ./logs/graph_unet/lift_joint*/*/best_model.pt 2>/dev/null | head -1)
fi
...
echo "========================================"
echo "Playing UnetPolicy + RL - Lift"
echo "RL: $CHECKPOINT"
