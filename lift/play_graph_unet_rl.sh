#!/bin/bash
# Play/Test GraphUnetPolicy + Residual RL - Lift task
cd "$(dirname "$0")/.."
set -e
...
if [ -z "$PRETRAINED_CHECKPOINT" ]; then
    PRETRAINED_CHECKPOINT=$(ls -t ./logs/graph_unet_full/lift_joint*/*/best_model.pt 2>/dev/null | head -1)
fi
...
echo "========================================"
echo "Playing GraphUnetPolicy + RL - Lift"
echo "RL: $CHECKPOINT"
