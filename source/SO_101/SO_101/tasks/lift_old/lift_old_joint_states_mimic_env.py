...
"""Environment wrapper for recording joint states directly for lift task.
...
from isaaclab.utils.math import matrix_from_quat, make_pose, quat_from_matrix
...
class SoArm100LiftJointStatesMimicEnv(ManagerBasedRLMimicEnv):
    """Isaac Lab Mimic environment wrapper for recording joint states directly for lift task.
    ...
    cfg: SoArm100LiftJointCubeEnvCfg
    ...
    def _setup_ik_controller(self):
        ...
        self._arm_joint_indices = [all_joint_names.index(name) for name in arm_joint_names]
        
        # Get body index for EE (fixed base robot, so -1)
        body_ids, _ = robot.find_bodies("end_effector")
        self._jacobi_body_idx = body_ids[0] - 1

    def _compute_ik_step(self, target_pos: torch.Tensor, target_quat: torch.Tensor, 
        ...
        # Get current state
        current_joint_pos = robot.data.joint_pos[env_id, self._arm_joint_indices]  # [num_arm_joints]
    ...
    def get_subtask_term_signals(self, env_ids: Sequence[int] | None = None) -> dict[str, torch.Tensor]:
        ...
        These signals indicate when the lift subtask is completed,
        ...
        # Subtask: lift_object - lift object to target height
        signals["lift_object"] = subtask_terms["lift_object"][env_ids]
        
        return signals

    def get_robot_eef_pose(self, eef_name: str = "end_effector", env_ids: Sequence[int] | None = None) -> torch.Tensor:
