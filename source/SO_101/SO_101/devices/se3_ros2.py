    ...
    def advance(self) -> torch.Tensor:
        ...
        if not self._is_connected():
            if not self._connection_warned:
                print(f"[Se3ROS2] ⚠️  No ROS2 messages received for {self.cfg.timeout_sec}s")
                print(f"[Se3ROS2] Make sure real robot is publishing to: {self.cfg.ee_pose_topic}")
    ...
    def _pose_callback(self, msg: Float64MultiArray):
        ...
        if self.cfg.input_format == "euler":
            ...
            if len(msg.data) >= 7:
                ...
                quat_wxyz = np.array([quat[3], quat[0], quat[1], quat[2]], dtype=np.float32)  # Convert to [w, x, y, z]
                ...
                print(f"[Se3ROS2] ⚠️  Received {len(msg.data)} elements, expected 7 for euler format")
            ...
            if len(msg.data) >= 8:
                ...
                print(f"[Se3ROS2] ⚠️  Received {len(msg.data)} elements, expected 8 for quat format")
