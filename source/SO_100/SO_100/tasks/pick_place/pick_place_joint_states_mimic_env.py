...
from isaaclab.utils.math import matrix_from_quat, make_pose, quat_from_matrix, compute_pose_error
...
class DualArmPickPlaceJointStatesMimicEnv(ManagerBasedRLMimicEnv):
    ...
    def get_subtask_term_signals(self, env_ids: Sequence[int] | None = None) -> dict[str, torch.Tensor]:
        ...
        # Subtask 1: push_cube - push cube to target
        # Subtask 2: lift_ee - lift hand up after push
        signals["push_cube"] = subtask_terms["push_cube"][env_ids]
        signals["lift_ee"] = subtask_terms["lift_ee"][env_ids]
        
        return signals

    def get_robot_eef_pose(self, eef_name: str = "dual_arm", env_ids: Sequence[int] | None = None) -> torch.Tensor:
    ...
    def get_object_poses(self, env_ids: Sequence[int] | None = None) -> dict[str, torch.Tensor]:
        ...
        cube_rot = matrix_from_quat(cube_quat)
        object_poses["cube"] = make_pose(cube_pos, cube_rot)
        ...
        tray_rot = matrix_from_quat(tray_quat)
        object_poses["object"] = make_pose(tray_pos, tray_rot)
