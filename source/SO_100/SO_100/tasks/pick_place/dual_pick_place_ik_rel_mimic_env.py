...
class DualArmPickPlaceIKRelMimicEnv(ManagerBasedRLMimicEnv):
    """
    Isaac Lab Mimic environment wrapper class for DualArm Pick Place IK Rel env.
    """
    ...
    def get_expected_attached_object(self, eef_name: str, subtask_index: int, env_cfg) -> str | None:
        ...
        # If placing, expect we are holding the object picked in the prior subtask
        if "place" in str(current_cfg.subtask_term_signal).lower():
            if subtask_index > 0:
                prev_cfg = subtask_configs[subtask_index - 1]
                if "pick" in str(prev_cfg.subtask_term_signal).lower():
