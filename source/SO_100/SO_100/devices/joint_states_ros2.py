...
class JointStatesROS2(DeviceBase):
    ...
    def __init__(self, cfg: JointStatesROS2Cfg):
        ...
        self._callbacks = {}
        self._lock = threading.Lock()  # Thread-safe access to latest positions
        ...
        # Subscribe to joint states topic with best-effort QoS for lower latency
        from rclpy.qos import QoSProfile, ReliabilityPolicy, DurabilityPolicy
        qos_profile = QoSProfile(
            depth=1,  # Keep only latest message
            reliability=ReliabilityPolicy.BEST_EFFORT,  # Lower latency, no guaranteed delivery
        ...
        print(f"[JointStatesROS2] 💡 Waiting for joint state data...")
    ...
    def advance(self) -> torch.Tensor:
        ...
        This method returns the latest joint positions from the background thread.
        ...
        if (self.cfg.enable_interpolation and
                current_time > 0 and prev_time > 0 and
        ...
        # No interpolation: return latest positions
    ...
    def _spin_ros2(self):
        """Spin ROS2 node in a separate thread to process callbacks.
        
        Uses continuous spinning for lowest latency.
        """
        while rclpy.ok():
            try:
                # Continuous spinning for lowest latency (no timeout)
                rclpy.spin_once(self.node, timeout_sec=0.0)
            except Exception as e:
                print(f"[JointStatesROS2] Error in ROS2 spin thread: {e}")
                break
    ...
    def _joint_state_callback(self, msg: JointState):
        ...
        try:
            ...
            # Update tensor directly (thread-safe)
            current_time = time.time()
            with self._lock:
