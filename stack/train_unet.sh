#!/bin/bash
# Train UnetPolicy (MLP encoder + U-Net) - Dual Cube Stack task
cd "$(dirname "$0")/.."
...
echo "========================================"
echo "Training UnetPolicy - Stack"
echo "Mode: $MODE | Joint FiLM: ${JOINT_FILM:-off}"
echo "Save: ./logs/graph_unet/$SUFFIX"
echo "========================================"
...
python scripts/graph_unet/train.py \
    ...
    --save_dir "./logs/graph_unet/$SUFFIX" \
    --log_dir "./logs/graph_unet/$SUFFIX" \
    $EXTRA_ARGS
