#!/bin/bash
# Play/Test UnetPolicy + Residual RL - Dual Cube Stack task
cd "$(dirname "$0")/.."
set -e
...
if [ -z "$CHECKPOINT" ] || [ ! -f "$CHECKPOINT" ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 ./logs/graph_unet_rl/.../policy_iter_200.pt"
    exit 1
fi

if [ -z "$PRETRAINED_CHECKPOINT" ]; then
    PRETRAINED_CHECKPOINT=$(ls -t ./logs/graph_unet/stack_joint*/*/best_model.pt 2>/dev/null | head -1)
fi

if [ -z "$PRETRAINED_CHECKPOINT" ] || [ ! -f "$PRETRAINED_CHECKPOINT" ]; then
    echo "Error: No pretrained UnetPolicy checkpoint found. Specify as 2nd arg."
    exit 1
fi
...
echo "========================================"
echo "Playing UnetPolicy + RL - Stack"
echo "RL: $CHECKPOINT"
