    ...
    if isinstance(obj, h5py.Group):
        ...
        if np.issubdtype(dtype, np.number):
            try:
                data = obj[:]
                if data.size > 0:
                    print(f"{prefix}   └─ min={data.min():.4f}, max={data.max():.4f}, mean={data.mean():.4f}, std={data.std():.4f}")
            ...
            if isinstance(obs_data, h5py.Dataset):
                if len(obs_data.shape) == 2 and step < obs_data.shape[0]:
                    ...
                    if len(obs_values) <= 10:
                        ...
                        print(f"    {obs_key}: {obs_values[:10]}... (shape: {obs_values.shape})")
    ...
    if 'actions' in demo:
        ...
        if actions.size > 0:
            try:
                ...
                print(f"   mean: {action_data.mean():.4f}, std: {action_data.std():.4f}")
    ...
    if 'rewards' in demo:
        ...
        if rewards.size > 0:
            try:
                ...
                print(f"   mean: {reward_data.mean():.4f}, sum: {reward_data.sum():.4f}")
            except Exception:
                pass
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python hdf5_reader.py <hdf5_file_path> [--interactive] [--demo N]")
        print("\nExample:")
        print("  python hdf5_reader.py datasets/dataset.hdf5  # Print first 5 demos, 10 steps each")
        print("  python hdf5_reader.py datasets/dataset.hdf5 --interactive  # Interactive mode")
        print("  python hdf5_reader.py datasets/dataset.hdf5 --demo 1  # Show complete demo 1")
        sys.exit(1)
            ...
            if 'data' in f:
                ...
                if demo_keys:
                    ...
                    if demo_lengths:
                        ...
                        print(f"📏 Average Demo Length: {np.mean(demo_lengths):.1f} steps")
                    ...
                    obs_container = first_demo.get('obs', first_demo.get('observations', None))
                    if obs_container:
                    ...
                    if 'actions' in first_demo:
                        ...
                        if actions.size > 0:
                            action_data = actions[:]
                            print(f"   dimension: {actions.shape[1] if len(actions.shape) > 1 else 1}")
                    ...
                    if print_all:
                        if demo_num is not None:
                            ...
                            if 1 <= demo_num <= len(demo_keys):
                                ...
                                print(f"❌ Error: Demo {demo_num} not found. Available demos: 1-{len(demo_keys)}")
                        else:
                            ...
                            try:
                                ...
                                if choice == '1':
                                    ...
                                    for i, demo_key in enumerate(demo_keys):
                                        demo = f[f'data/{demo_key}']
                                        length = demo['actions'].shape[0] if 'actions' in demo else 0
                                        print(f"\n  Demo {i+1}: {demo_key}")
                                        print(f"    Length: {length} steps")
                                        if 'meta' in demo:
                                            meta_items = list(demo['meta'].attrs.items())
                                            if meta_items:
                                                print(f"    Metadata: {dict(meta_items)}")
                                
                                elif choice == '2':
                                    print(f"\nAvailable demos (1-{len(demo_keys)}):")
                                    demo_num = input(f"Enter demo number (1-{len(demo_keys)}): ").strip()
                                    try:
                                        idx = int(demo_num) - 1
                                        if 0 <= idx < len(demo_keys):
                                            browse_demo(f, demo_keys[idx], idx, len(demo_keys))
                                        else:
