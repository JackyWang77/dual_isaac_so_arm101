    ...
    def collect_rollout(self) -> Dict[str, float]:
        ...
        for step in range(self.steps_per_env):
            ...
            with torch.no_grad():
                ...
                if self._is_dual_arm and action_dim == 12:
                    # Dual arm: EMA for joints [0:5, 6:11], gripper mapping for [5, 11]
                    ...
                    for i, idx in enumerate(joint_indices):
                        action_for_sim[:, idx] = self._ema_smoothed_joints[:, i]
                    # Gripper: linear mapping (same as play.py for stack task)
                    for g_idx in gripper_indices:
                        joint_val = action_for_sim[:, g_idx].float()
                        action_for_sim[:, g_idx] = 12.5 * joint_val + 0.5  # gripper_a=12.5, gripper_b=0.5
                    # CRITICAL: Reorder [left_6, right_6] → [right_6, left_6] for env
...
def main():
    ...
    # Auto-detect: dual arm backbone → 12D, single arm → 6D
    _backbone_action_dim = getattr(backbone_policy.cfg, "action_dim", 6)
    is_dual_arm = _backbone_action_dim == 12 or getattr(backbone_policy.cfg, "arm_action_dim", None) is not None
    action_dim_rl = 12 if is_dual_arm else 6

    print(f"[Main] obs_dim: {obs_dim}, action_dim_env: {action_dim_env}, action_dim_rl: {action_dim_rl}")
    print(f"[Main] {'Dual' if is_dual_arm else 'Single'} arm mode")
