...
"""
Standalone HDF5 demo dataset and collate for Graph-Unet (6-dim: 5 arm + 1 gripper).

No SO_101 / Isaac Sim dependencies so scripts like visualize_loss_landscape.py
can run with plain Python (no omni.physics). Same structure as graph_dit/dataset.py
but keeps full 6-dim actions (no arm-only slice).
"""
...
class HDF5DemoDataset(Dataset):
    ...
    Same as Graph-DiT dataset but action_dim = 6 (5 arm + 1 gripper) for Graph-Unet.
    """
    ...
    def __init__(
        ...
        with h5py.File(hdf5_path, "r") as f:
            ...
            for demo_key in demo_keys:
                ...
                # Graph-Unet: keep full 6-dim (5 arm + 1 gripper), no slice
