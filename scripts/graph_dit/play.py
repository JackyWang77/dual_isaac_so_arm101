...
def play_graph_dit_policy(
    ...
    if joint_stats is not None:
        ...
        print(f"[Play] Warning: No joint stats found, joint states will NOT be normalized!")
        print(f"[Play] This may cause poor performance - retrain with joint normalization!")
        joint_mean = None
        joint_std = None
    ...
    with torch.inference_mode():
        while simulation_app.is_running() and episode_count < num_episodes:
            ...
            if joint_mean is not None and joint_std is not None and joint_states_history_tensor is not None:
                joint_states_history_tensor = (joint_states_history_tensor - joint_mean) / joint_std
