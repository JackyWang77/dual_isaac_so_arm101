...
class HDF5DemoDataset(Dataset):
    ...
    def __init__(self, hdf5_path: str, obs_keys: list[str], normalize_obs: bool = True, 
        ...
        with h5py.File(hdf5_path, 'r') as f:
            ...
            for demo_key in demo_keys:
                ...
                if not self.obs_key_dims:
                    ...
                    for key in obs_keys:
                        if key in obs_container:
                            ...
                            offset += key_dim
                            print(f"[HDF5DemoDataset] Key '{key}': dim={key_dim}, offset={self.obs_key_offsets[key]}")
                ...
                trajectory_mask_seq = []  # [T, pred_horizon] - True if valid, False if padding
                for i in range(T):
                    traj = []
                    traj_mask = []
                ...
                action_trajectory_seq = np.stack(action_trajectory_seq, axis=0)  # [T, pred_horizon, action_dim]
                trajectory_mask_seq = np.stack(trajectory_mask_seq, axis=0)  # [T, pred_horizon]
                ...
                action_history_seq = np.stack(action_history_seq, axis=0)  # [T, history_len, action_dim]
                ...
                for i in range(T):
                    ...
                    for j in range(max(0, i - self.action_history_length + 1), i + 1):
                        ...
                        obj_pos = obs_j[self.obs_key_offsets.get('object_position', 12):
                                       self.obs_key_offsets.get('object_position', 12) + 3]
                        obj_ori = obs_j[self.obs_key_offsets.get('object_orientation', 15):
                                       self.obs_key_offsets.get('object_orientation', 15) + 4]
                        ee_pos = obs_j[self.obs_key_offsets.get('ee_position', 19):
                                      self.obs_key_offsets.get('ee_position', 19) + 3]
                        ee_ori = obs_j[self.obs_key_offsets.get('ee_orientation', 22):
                ...
                ee_node_history_seq = np.stack(ee_node_history_seq, axis=0)  # [T, history_len, 7]
                object_node_history_seq = np.stack(object_node_history_seq, axis=0)  # [T, history_len, 7]
                ...
                for i in range(T):
                    ...
                    for j in range(max(0, i - self.action_history_length + 1), i + 1):
                        ...
                        joint_vel = np.pad(joint_vel, (0, max(0, jv_dim - len(joint_vel))))[:jv_dim]
                        joint_hist.append(np.concatenate([joint_pos, joint_vel]).astype(np.float32))
                    
                    while len(joint_hist) < self.action_history_length:
                        joint_hist.insert(0, joint_hist[0].copy() if joint_hist else np.zeros(jp_dim + jv_dim, dtype=np.float32))
                    
                    joint_states_history_seq.append(np.stack(joint_hist, axis=0))
                joint_states_history_seq = np.stack(joint_states_history_seq, axis=0)  # [T, history_len, joint_dim]
                    ...
                    if 'subtask_term_signals' in datagen_info:
                        ...
                        for i in range(T):
                            ...
                            for idx, name in enumerate(self.subtask_order):
                                if name in signals_group and not np.array(signals_group[name])[i]:
                                    active_idx = idx
                                break
                        ...
                        subtask_condition_seq = np.stack(subtask_condition_seq, axis=0)  # [T, num_subtasks]
                ...
                all_ee_nodes.append(ee_node_history_seq[:, -1, :])  # Current timestep nodes
                all_object_nodes.append(object_node_history_seq[:, -1, :])
                all_joint_states.append(joint_states_history_seq[:, -1, :])  # Current timestep joints
        ...
        if self.single_batch_test:
            if len(self.demos) == 0:
                raise ValueError("[SINGLE BATCH TEST] No demos loaded! Cannot create single batch test.")
            ...
            print(f"  ✅ Single batch dataset created: {len(self.demos)} identical demos")
        ...
        print(f"  Demo lengths: min={min(demo_lengths)}, max={max(demo_lengths)}, mean={np.mean(demo_lengths):.1f}")
        print(f"  Obs stats: mean shape={self.obs_stats.get('mean', np.array([])).shape}")
        print(f"  Action stats: mean shape={self.action_stats.get('mean', np.array([])).shape}")
        ...
        if len(self.action_stats) > 0:
            ...
            print(f"  Action Mean: min={action_mean.min():.6f}, max={action_mean.max():.6f}, mean={action_mean.mean():.6f}")
            print(f"  Action Std:  min={action_std.min():.6f}, max={action_std.max():.6f}, mean={action_std.mean():.6f}")
            
            # Check for anomalies
            if (action_std < 1e-6).any():
                print(f"  ⚠️  WARNING: Action Std has values < 1e-6! This may cause numerical issues!")
            if (action_std > 100).any():
                print(f"  ⚠️  WARNING: Action Std has values > 100! Normalization may be too aggressive!")
            if (action_std < 0.01).any():
                print(f"  ⚠️  WARNING: Action Std has very small values (< 0.01). Check if this dimension is constant!")
        
        if len(self.obs_stats) > 0:
            obs_mean = self.obs_stats['mean']
            obs_std = self.obs_stats['std']
            print(f"  Obs Mean: min={obs_mean.min():.6f}, max={obs_mean.max():.6f}, mean={obs_mean.mean():.6f}")
            print(f"  Obs Std:  min={obs_std.min():.6f}, max={obs_std.max():.6f}, mean={obs_std.mean():.6f}")
            
            if (obs_std < 1e-6).any():
                print(f"  ⚠️  WARNING: Obs Std has values < 1e-6! This may cause numerical issues!")
            if (obs_std > 100).any():
                print(f"  ⚠️  WARNING: Obs Std has values > 100! Normalization may be too aggressive!")
    ...
    def __getitem__(self, idx):
        ...
        subtask_condition_seq = demo['subtask_condition_seq'].copy() if demo['subtask_condition_seq'] is not None else None
        ...
        if self.normalize_actions and len(self.action_stats) > 0:
            ...
            action_trajectory_seq = (action_trajectory_seq - self.action_stats['mean']) / self.action_stats['std']
            action_history_seq = (action_history_seq - self.action_stats['mean']) / self.action_stats['std']
        
        # Normalize node features: [T, hist_len, 7]
        if len(self.node_stats) > 0:
            ee_node_history_seq = (ee_node_history_seq - self.node_stats['ee_mean']) / self.node_stats['ee_std']
            object_node_history_seq = (object_node_history_seq - self.node_stats['object_mean']) / self.node_stats['object_std']
        
        # CRITICAL: Normalize joint states: [T, hist_len, joint_dim]
        if len(self.joint_stats) > 0:
            joint_states_history_seq = (joint_states_history_seq - self.joint_stats['mean']) / self.joint_stats['std']
        
        return {
            'obs_seq': torch.from_numpy(obs_seq).float(),  # [T, obs_dim]
            'action_seq': torch.from_numpy(action_seq).float(),  # [T, action_dim]
            'action_trajectory_seq': torch.from_numpy(action_trajectory_seq).float(),  # [T, pred_horizon, action_dim]
            'action_history_seq': torch.from_numpy(action_history_seq).float(),  # [T, hist_len, action_dim]
            'ee_node_history_seq': torch.from_numpy(ee_node_history_seq).float(),  # [T, hist_len, 7]
            'object_node_history_seq': torch.from_numpy(object_node_history_seq).float(),  # [T, hist_len, 7]
            'joint_states_history_seq': torch.from_numpy(joint_states_history_seq).float(),  # [T, hist_len, joint_dim]
            'subtask_condition_seq': torch.from_numpy(subtask_condition_seq).float() if subtask_condition_seq is not None else None,
            'length': T,
        }
...
def train_graph_dit_policy(
    ...
    print(f"[Train] CRITICAL: Each sample is a complete demo sequence (not individual timesteps)")
    # ============================================================================
    ...
    if single_batch_test:
        ...
        print(f"  This will use ONLY the first demo, replicated {single_batch_size} times")
        print(f"  Expected outcome: Loss should drop to < 0.001 if code logic is correct")
        print(f"  If loss stays > 0.05, there's likely a bug in model architecture or data flow\n")
    ...
    print(f"[Train] Action trajectory shape: [T, {actual_pred_horizon}, {actual_action_dim}]")
    ...
    print(f"[Train] (Each demo has ~{sample_T} timesteps, so effective batch ≈ {demo_batch_size * sample_T} timesteps)")
    ...
    if lr_schedule == "cosine":
        ...
        print(f"[Train] Using COSINE schedule: {warmup_epochs} warmup epochs ({100*warmup_epochs/num_epochs:.1f}%), then cosine annealing")
    else:
        # Constant learning rate (stable, no scheduling)
        # This is more reliable for debugging and initial training
        scheduler = None
        print(f"[Train] Using CONSTANT learning rate: {learning_rate} (no warmup, no decay)")
    ...
    print(f"[Train] Loss is computed over all timesteps in each demo, with padding masked out")
    policy.train()
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            obs_seq = batch['obs_seq'].to(device, non_blocking=True)  # [B, max_T, obs_dim]
            action_trajectory_seq = batch['action_trajectory_seq'].to(device, non_blocking=True)  # [B, max_T, pred_horizon, action_dim]
            action_history_seq = batch['action_history_seq'].to(device, non_blocking=True)  # [B, max_T, hist_len, action_dim]
            ee_node_history_seq = batch['ee_node_history_seq'].to(device, non_blocking=True)  # [B, max_T, hist_len, 7]
            object_node_history_seq = batch['object_node_history_seq'].to(device, non_blocking=True)  # [B, max_T, hist_len, 7]
            joint_states_history_seq = batch['joint_states_history_seq'].to(device, non_blocking=True)  # [B, max_T, hist_len, joint_dim]
            subtask_condition_seq = batch['subtask_condition_seq']
            if subtask_condition_seq is not None:
                subtask_condition_seq = subtask_condition_seq.to(device, non_blocking=True)  # [B, max_T, num_subtasks]
            lengths = batch['lengths']  # [B] - original lengths
            mask = batch['mask'].to(device, non_blocking=True)  # [B, max_T] - True for valid timesteps
            trajectory_mask = batch['trajectory_mask'].to(device, non_blocking=True)  # [B, max_T, pred_horizon] - horizon-level mask
            ...
            actions_flat = action_trajectory_seq.reshape(B * max_T, pred_horizon, -1)  # [B*max_T, pred_horizon, action_dim]
            action_history_flat = action_history_seq.reshape(B * max_T, action_history_length, -1)  # [B*max_T, hist_len, action_dim]
            ee_node_history_flat = ee_node_history_seq.reshape(B * max_T, action_history_length, -1)  # [B*max_T, hist_len, 7]
            object_node_history_flat = object_node_history_seq.reshape(B * max_T, action_history_length, -1)  # [B*max_T, hist_len, 7]
            joint_states_history_flat = joint_states_history_seq.reshape(B * max_T, action_history_length, -1)  # [B*max_T, hist_len, joint_dim]
            subtask_condition_flat = None
            if subtask_condition_seq is not None:
                subtask_condition_flat = subtask_condition_seq.reshape(B * max_T, -1)  # [B*max_T, num_subtasks]
            trajectory_mask_flat = trajectory_mask.reshape(B * max_T, pred_horizon)  # [B*max_T, pred_horizon] - horizon-level mask
            ...
            if batch_idx == 0 and epoch % 100 == 0:  # Print every 100 epochs
                ...
                if 'debug' in loss_dict:
                    ...
                    if 'v_pred' in debug and 'v_t' in debug:
                        ...
                        print(f"  Target (v_t) Mean: {v_t.mean().item():.4f}, Std: {v_t.std().item():.4f}")
                        print(f"  Target (v_t) Min: {v_t.min().item():.4f}, Max: {v_t.max().item():.4f}")
                        print(f"  Pred (v_pred) Mean: {v_pred.mean().item():.4f}, Std: {v_pred.std().item():.4f}")
                        print(f"  Pred (v_pred) Min: {v_pred.min().item():.4f}, Max: {v_pred.max().item():.4f}")
                        print(f"  Input Action (Data) Min: {actions.min().item():.4f}, Max: {actions.max().item():.4f}")
                        ...
                        if 't' in debug:
                            t = debug['t']
                            print(f"  Time t: Min={t.min().item():.4f}, Max={t.max().item():.4f}, Mean={t.mean().item():.4f}")
                        ...
                        print(f"[DDPM Debug]")
                        print(f"  Target Noise Mean: {noise.mean().item():.4f}, Std: {noise.std().item():.4f}")
                        print(f"  Pred Noise Mean: {noise_pred.mean().item():.4f}, Std: {noise_pred.std().item():.4f}")
                        print(f"  Input Action (Data) Min: {actions.min().item():.4f}, Max: {actions.max().item():.4f}")
                    ...
                    print(f"  Actions range: [{actions_flat.min().item():.4f}, {actions_flat.max().item():.4f}]")
