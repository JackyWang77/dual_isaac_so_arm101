    ...
    def __init__(
        ...
        with h5py.File(hdf5_path, "r") as f:
            ...
            for demo_key in demo_keys:
                ...
                # Load original actions from HDF5 (will be replaced with joint_pos[t+2])
                # Original actions from teleoperation may have large gaps, so we use joint_pos[t+2] instead
                ...
                # CRITICAL FIX: Replace actions with joint_pos[t+2] for smoother actions
                # Teleoperation data has large gaps between action and current joint_pos,
                # so using joint_pos[t+2] as action produces smoother, more learnable actions
                ...
                # Replace actions with joint_pos[t+2] (2 timesteps ahead)
                # For timestep i, action = joint_pos[i+2] (if exists) or last available joint_pos
                actions_new = []
                for i in range(T):
                    target_idx = i + 2
                    if target_idx < T:
                        # Use joint_pos 2 steps ahead as action
                ...
                print(f"[HDF5DemoDataset] ✅ Replaced actions with joint_pos[t+2] for smoother actions")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Action dimension (now uses joint_pos[t+2], typically 6 for 6-DoF arm)",
    )
